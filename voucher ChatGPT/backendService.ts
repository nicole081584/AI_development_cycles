...
/**
 * Purchases a voucher by sending customer and voucher details to the backend.
 *
 * @param voucherDetails   object containing title, firstName, lastName,
 *                         phoneNumber, email and value
 * @returns                created voucher data if successful, otherwise null
 */
export async function purchaseVoucher(voucherDetails: {
  title: string;
  firstName: string;
  lastName: string;
  phoneNumber: string;
  ...
  console.log("Requesting voucher purchase with:", voucherDetails);
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        firstName: voucherDetails.firstName,
        lastName: voucherDetails.lastName,
        phoneNumber: voucherDetails.phoneNumber,
        email: voucherDetails.email,
    ...
    console.log("Raw voucher purchase response:", text);
    ...
    const voucherArray = Array.isArray(data) ? data : [data];

if (voucherArray.length === 0) {
  console.log("Voucher purchase returned no voucher data");
  return null;
}

const parsedVouchers = parseVoucher(voucherArray);

const purchasedVoucher = parsedVouchers[0];

console.log("Mapped purchased voucher:", purchasedVoucher);

return purchasedVoucher;
  } catch (error: any) {
    console.error("Voucher purchase failed:", error);
    alert("Error purchasing voucher: " + (error.message || String(error)));
    return null;
  }
}
